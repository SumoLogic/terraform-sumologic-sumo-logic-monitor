...
module "SQLServer-AppDomain" {
  ...
  monitor_description         = "This alert fires when we detect that the SQL Server AppDomain."
...
module "SQLServer-InstanceDown" {
  ...
  email_notifications       = var.email_notifications
  queries = {
    A = "${var.sqlserver_data_source} db_system=sqlserver db_cluster=* metric=sqlserver_server_properties_uptime "
  }
  triggers = [
			  {
